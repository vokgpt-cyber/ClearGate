...
CLEARGATE использует нативные SDK для каждого LLM-провайдера (см. ADR-0006). Эта задача — реализация ClaudeAdapter с полной поддержкой extended thinking, стриминга и cost estimation. Это первый адаптер; OpenAI и Gemini делаются по аналогии в отдельных задачах.
