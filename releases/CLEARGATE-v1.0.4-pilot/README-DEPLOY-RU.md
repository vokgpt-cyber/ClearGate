...
Для hotfix `pilot.12` важно: пересобрать и backend, и frontend. Backend получил
policy layer `auto/review/ignore`, а frontend больше не применяет Deep Scan
автоматически. После обновления проверьте:

```bash
bash releases/CLEARGATE-v1.0.4-pilot/scripts/smoke-test.sh
```

Затем на тестовом документе:

1. ДМС/страховая памятка: ФИО и номер полиса скрываются, support-телефоны и
   города офисов не превращаются в сущности.
2. Внутреннее положение: `Адвокатское бюро ЕПАМ` скрывается, но родовые слова
   `Сотрудник`, `Адвокатское бюро`, `Бюро`, публичные AI-сервисы не скрываются.
3. Deep Scan показывает предложения отдельным списком и не меняет документ до
...
инструкции он описан как дополнительная проверка. Начиная с `pilot.12` Deep
Scan не меняет разметку автоматически: он показывает QA-предложения, которые
пользователь должен выбрать вручную.
...
- применять только те предложения, которые действительно нужны;
- при спорном результате оставить предложения выключенными.
