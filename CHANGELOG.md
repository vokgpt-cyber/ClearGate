...
### Fixed
- Добавлен policy layer `auto/review/ignore` поверх NER-распознавания: в
  страховых/DMS документах и внутренних положениях родовые слова вроде
  `Сотрудник`, `Адвокатское бюро`, публичные сервисы, support-телефоны и города
  офисов больше не должны попадать в автоматическую анонимизацию, а ФИО, номера
  полисов, даты и конкретные фирмы вроде `Адвокатское бюро ЕПАМ` сохраняются.
- Deep Scan больше не применяет предложения автоматически: verifier теперь
  выводит QA-кандидаты в отдельный список, а документ меняется только после
