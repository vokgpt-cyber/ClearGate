...
_PROFESSIONAL_FORM_RE = re.compile(
    r"\b(?:"
    r"адвокатск\w+\s+бюро|адвокатск\w+\s+контор\w+|коллеги\w+\s+адвокатов|"
    r"юридическ\w+\s+фирм\w+|юридическ\w+\s+бюро|патентн\w+\s+бюро|law\s+firm"
    r")\b",
    re.IGNORECASE,
)
...
def _has_professional_context_before(text: str, entity: DetectedEntity) -> bool:
    ...
    return bool(
        re.search(
            r"(?:адвокатск\w+\s+бюро|юридическ\w+\s+фирм\w+|коллеги\w+\s+адвокатов)\s*\"?$",
            before,
        )
    )
