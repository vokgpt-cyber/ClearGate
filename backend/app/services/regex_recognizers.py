...
class OrganizationRuRecognizer(GroupRegexRecognizer):
    ...
    PATTERNS = [
        ...
        (
            re.compile(
                (
                    r"(?P<value>\b(?:Адвокатск\w+\s+бюро|Адвокатск\w+\s+контор\w+|"
                    r"Коллеги\w+\s+адвокатов|Юридическ\w+\s+фирм\w+|"
                    r"Юридическ\w+\s+бюро|Патентн\w+\s+бюро)\s+"
                    r"(?:[«\"][^»\"]{2,120}[»\"]|(?-i:[А-ЯЁA-Z])[А-ЯЁA-Zа-яёA-Za-z0-9&'’\-]{2,}"
                    r"(?:\s+(?-i:[А-ЯЁA-Z])[А-ЯЁA-Zа-яёA-Za-z0-9&'’\-]{1,}){0,5}))"
                ),
                _REGEX_FLAGS,
            ),
            0.91,
